        ...
        for i, entry in enumerate(chords):
            if not isinstance(entry, dict):
                raise ValueError(
                    ...
                    f"{context}[{i}] must be an object"
                )
            ts = entry.get("timestamp")
            ch = entry.get("chord")
            if not ChordTrackRepository._is_finite_number(ts) or ts < 0:
                raise ValueError(
                    f"Invalid chord data in {file_path}: "
    ...
    def __validate_rhythm_entry(entry, file_path, context):
        for required in ("bpm", "meter_signature", "beat_times", "beat_numbers"):
            if required not in entry:
                raise ValueError(
                    f"Invalid chord data in {file_path}: "
                    f"{context} must contain \"{required}\""
                )

        bpm = entry["bpm"]
        if bpm is not None and (
            not ChordTrackRepository._is_finite_number(bpm) or bpm <= 0
        ):
            raise ValueError(
                f"Invalid chord data in {file_path}: "
                f"{context} bpm must be positive, got {bpm!r}"
            )

        meter = entry["meter_signature"]
        if meter is not None and (
            not isinstance(meter, int) or isinstance(meter, bool) or meter <= 0
        ):
            raise ValueError(
                f"Invalid chord data in {file_path}: "
        ...
        for i, bn in enumerate(beat_numbers):
            if (
                not isinstance(bn, int)
                or isinstance(bn, bool)
                or bn <= 0
            ):
                raise ValueError(
